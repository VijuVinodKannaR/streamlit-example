...
def google_chrome():
    try:
        ...
        subprocess.run(["cmd","/c","winget", "upgrade", "Chrome", "--disable-interactivity"], check=True)
...
def microsoft_edge():
    try:
        # Execute the PowerShell command to upgrade the application as admin
        subprocess.run(["cmd","/c","winget", "upgrade", "Edge", "--disable-interactivity"], check=True)
        st.success("Application updated successfully.")
    except subprocess.CalledProcessError as e:
        st.error(f"No New Updates Available")
def microsoft_teams():
    try:
        # Execute the PowerShell command to upgrade the application as admin
        subprocess.run(["cmd","/c","winget", "upgrade", "Teams", "--disable-interactivity"], check=True)
        st.success("Application updated successfully.")
    except subprocess.CalledProcessError as e:
        st.error(f"No New Updates Available")
